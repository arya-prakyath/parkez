...
export default function userProfile({ setTitle, setScreen }: userProfileProps) {
    ...
    const [address, setAddress] = useState('Rajajinagara, Bangalore, Karnataka, India - 560 079');
    const [fileResponse, setFileResponse] = useState();

    const uploadImage = () => {
        DocumentPicker.getDocumentAsync().then(file => file).then(result => {
            if (result.type === "success"){
                if (['jpg', 'jpeg', 'png'].includes(result.name.split('.').pop() ?? "")){
                    setProfileImageData(result.uri);
                    ToastAndroid.show("Profile picture changed", 100);
                }
                else{
                    ToastAndroid.show("Please choose a valid image ( JPG, JPEG, PNG)", 100);
                }
            }
            else{
    ...
    return (
        <View style={styles.container}>
            <TouchableOpacity style={styles.profileImageContainer} onPress={() => uploadImage()}>
                {profileImageData ? (
                    <Image
...

                {
                    editMode ? (
                        <View style={styles.buttonContainer}>
                            <TouchableOpacity style={styles.homeButton} onPress={() => setEditMode(false)}>
                                <Image
